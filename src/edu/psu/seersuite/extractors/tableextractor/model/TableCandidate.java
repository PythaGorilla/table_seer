...
public class TableCandidate {
    private boolean m_valid = true;                    //whether this table candidate is a valid table or not
    private boolean m_WideTable = false;               //whether this table is a wide table or not
    private boolean m_topCaptionLocation = true;       //whether the caption of the table is on the top of the table body or not
    ...
    private String m_metadata_HighLevel = "";          //the metadata of this table in the high-level (without the table structure decomposition result)
    private String m_metadata_StructureLevel = "";     //the metadata of this table in the structure-level (with the table structure decomposition result)  
    private String m_rowHeadings = "";                 //the heading rows of a table candidate
    private String m_columnHeadings = "";              //the heading columns of a table candidate
    
    private String m_classficationData;                //the classification data for further table understanding  
    private String m_referenceText = "";               //the reference text in the document of a table candidate
    ...
    private int m_captionStartLine;                    //the line ID of the first table caption line in the document page
    private int m_captionEndLine;                      //the line ID of the last table caption line in the document page
    private int m_bodyStartLine;                       //the line ID of the first table body line in the document page
    private int m_bodyEndLine;                         //the line ID of the last table body line in the document page
    private int m_footnoteBeginRow = -1;               //the line ID of the first table footnote line in the document page, if appliable
    private int m_headingLineNumber = 0; 	           //the number of combined lines in the heading rows
    ...
    private float m_minGapBtwColumns = 1000.0f;        //the minimum gaps between every two neighbor table columns 
    ...
    public boolean[][] m_crossCells;                   //the cross row/column table cells (nesting cells)
    private float[] m_leftX_tableColumns;              //the left-end X axis of each table column
    private float[] m_rightX_tableColumns;             //the right-end X axis of each table column
    ...
    */
    public void setHighLevelMeta(int i) {
    	...
    	String location = "";
    	if (m_topCaptionLocation) location="top";
    	else location="below";
    	...
    	meta = meta + "<dataHighLevel>" + m_metadata_HighLevel + "</dataHighLevel>\n";
    	this.setMetadataHighLevel(meta);
    }
    
    /*
    public void setHighLevelMetaX(int i){
    	Document xmldoc = this.xml_doc;
    	Element root = xmldoc.getRootElement();
    	
    	Element subroot = new Element("Table");
    	subroot.addContent(new Element("pageNumInDoc").addContent(Integer.toString(i+1)));
    	subroot.addContent(new Element("TableCaption").addContent(m_caption));
    	if (m_topCaptionLocation) subroot.addContent(new Element("CaptionLocation").addContent("top"));
    	else subroot.addContent(new Element("CaptionLocation").addContent("below"));
    	subroot.addContent(new Element("dataHighLevel").addContent(m_metadata_HighLevel));
    ...
    */
    /*
    public void setMetaStructureLevelX(int YNum, int cc, ArrayList<TextPiece> wordsOfAPage, DocInfo m_docInfo){
    ...
    */
     ...
     */
    public void setMetadataStructureLevel(int YNum, int cc, ArrayList<TextPiece> wordsOfAPage, DocInfo m_docInfo) {
		String rowHeading = this.getRowHeadings();
		int heads = this.getHeadingLineNumber();
    	int footnoteLineIndex = this.getFootnoteBeginRow();
    	int MAXcolumnNum = this.getMaxColumnNumber();
		...
		detailedTableMeta = detailedTableMeta + "<TableRowHeading>" + m_rowHeadings + "</TableRowHeading>" + "\n";
		...
		detailedTableMeta = detailedTableMeta + m_footnoteText  + "... </TableFootnote>"  + "\n";
		...
		this.m_height = wordsOfAPage.get(this.getBodyEndLine()).getEndY() - wordsOfAPage.get(this.getBodyStartLine()).getY(); 
		detailedTableMeta = detailedTableMeta + "<TableHeight>"  + this.m_height + "</TableHeight>" + "\n";
		
		this.m_width = rightX_tableColumns[cc-1] - leftX_tableColumns[0];
		detailedTableMeta = detailedTableMeta + "<TableWidth>"  + this.m_width + "</TableWidth>" + "\n";
		if (m_WideTable==false) {	//fix the wideTableOrNot based on the width
			if (rightX_tableColumns[cc-1]>(m_docInfo.getMiddleX()) && leftX_tableColumns[0]<m_docInfo.getMiddleX()) 
				m_WideTable = true;
		}
		detailedTableMeta =detailedTableMeta + "<isWideTable>" + m_WideTable + "</isWideTable>\n";
		m_referenceText = replaceAllSpecialChracters(m_referenceText);
		detailedTableMeta = detailedTableMeta + "<TableReferenceText>" + m_referenceText + "</TableReferenceText>" + "\n";
     ...
     *          an object of DocInfo
     */
    public void setEmptyMetadataStructureLevel(int YNum, int cc, ArrayList<TextPiece> wordsOfAPage, DocInfo m_docInfo) {
		String detailedTableMeta = "";
		detailedTableMeta = "<TableColumnHeading>\n" + "</TableColumnHeading>" + "\n";
   ...
   //System.out.printf("-------------------------\n");
	...
	/*
	System.out.printf(
		"caption\t[line#%d, line#%d]\t[x%f, x%f]\n",
		this.getCaptionStartLine(),
		this.getCaptionEndLine(),
		this.getCaptionX(),
		this.getCaptonEndX());
    System.out.printf(

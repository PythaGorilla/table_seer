    	...
    	if (m_parser == null) {
    		...
    		if (wordsByPage != null && wordsByPage.size()!=0 ) {	//in order to keep the fast speed for large repositories, we do not process PDF files with 150+ pages
    			...
    			if (pageNum<=150) {
    				...
    				if (unknownCodedDoc) {
    					unknownCodedPDF ++;
    					m_docInfo.setErrorMsg("Sorry, this PDF document is coded with the special coders, which PDFBOX can not process successfully.");
    				...
    				if (isHTMLCode) {
    					HTMLCodePDF++;
    					m_docInfo.setErrorMsg("Sorry, this PDF document is coded in HTML code. TableSeer does not process such files now.");
    				 ...
    				 */
    				//if (unknownCodedDoc==false && isHTMLCode==false) { //too much PDF files will be filtered out using this condition
    				if (isHTMLCode==false) { 
    					...
    					for (ArrayList<TextPiece> linesOfAPage : linesByPage) {
    						...
    						//extractFiguresFromAPage(linesOfAPage, wordsOfAPage, i, pdfFile, linesByPage);
    						i++;
    					}
    					meta = m_docInfo.getTableMetaInThisDoc();    				
    					meta = meta + "<errorMsg>" + m_docInfo.getErrorMsg() + "</errorMsg>\n";
    					meta = meta + "</Tables>\n";
    					...
    					xml_root.addContent(new Element("errorMsg").addContent(m_docInfo.getErrorMsg()));
    					...
    					if (Config.DEBUG_MODE) 	{	//print table metadata ordered by documents
    						...
    						Debug.printStatInfo(outputDirPath, pdfFile, pageNum, m_docInfo.getTableNum());
    					}
        			...
        			m_docInfo.setErrorMsg("Sorry, for the scalability and speed reasons, we do not process the PDF documents that are TOO LARGE!");
        			return null;
    			}
    		}
    		else {
    			...
    			m_docInfo.setErrorMsg("Sorry, PDFBOX can not extract any text from this PDF document!");
    			return null;
    		}
       	}//end else()
    }// extract()
    	...
    	if (linesOfAPage.size() > 30){
    		...
    		if (tableKwdExist == true)    {
    			for (int k = 0; k < linesOfAPage.size();) {
    				...
    				if (keyword != null && (textWithoutSpace.startsWith("tableofcontent")==false) && (textWithoutSpace.startsWith("TABLEOFCONTENT")==false)&& (textWithoutSpace.startsWith("TableofContent")==false)) {
    					...
    					if (prevLine != null && line.getY() == prevLine.getY()) k++;
    						...
    						//tc.setXMLDoc(xml_doc); // to add all table metadata elements in TableCandidate
   ...
   */
    ...
    private int findCaptionBoundary(int lineNumber, ArrayList<TextPiece> linesOfAPage, TableCandidate tc){
    	...
    	while (lineNumber < linesOfAPage.size()) {
    		...
    		//Todo caption area detection parameter tuning , was 1.35 , now 0.85
			if (((yDiff <= (m_docInfo.getAverageLineGap() * 0.85))  //case 2: the next long caption line
					&& (yDiff > 0.0)
				...
				lineNumber++;
			} else if (((currentLine.getX() < prevLine.getEndX())    //case 4: the last short caption line
					&& (currentLine.getWidth() < ((m_docInfo.getMaxX() - m_docInfo.getMinX()) / 4.0))
					&& (yDiff > 0)
					&& (yDiff <= (m_docInfo.getAverageLineGap() * 0.85))
				...
				lineNumber++;
			} else if ((yDiff == 0.0) || ((Math.abs(yDiff) < (m_docInfo.getAverageLineGap() / 2.0))        //case 3:
